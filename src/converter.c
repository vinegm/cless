...
static void placePiece(boardState *board, char piece, uint64_t mask);
...
boardState fenToBitboard(const char *fen) {
  ...
  for (const char *piece = fen; *piece != ' '; piece++) {
    if (*piece == '/') {
      row++;
      col = 0;
      continue;
...
void placePiece(boardState *board, char piece, uint64_t mask) {
  switch (piece) {
    case 'p':
