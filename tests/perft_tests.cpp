...
void test_perft_position(ClessEngine &board,
                         const std::vector<uint64_t> &expected_results) {
  for (int depth = 1; depth <= expected_results.size(); depth++) {
    uint64_t nodes = board.perft(depth);
    cr_assert_eq(nodes, expected_results[depth - 1],
                 "Perft mismatch at depth %d: got %lu, expected %lu", depth,
                 nodes, expected_results[depth - 1]);
  }
}
